...
2. [`scripts/update_versions.py`](scripts/update_versions.py) (nur Stdlib + Docker):
   ...
   - entdeckt zusätzlich **neue datierte Snapshot-Tags** (z. B.
     `25.2026.07.11.03-x86_64`), die seit dem letzten Lauf erstellt wurden
   ...
   - nur wenn ein Digest neu/unbekannt ist: `docker pull` + `java -version` im
     Container, Parsen der Corretto-Version. Ist ein Digest bereits bekannt
...
💡 **Click-to-copy:** Auf der Website kopiert ein Klick auf einen Tag oder Digest den
gepinnten Verweis `public.ecr.aws/lambda/java:<tag>@sha256:<amd64-digest>` in die
Zwischenablage – genau so, wie er in einem Dockerfile hinter `FROM` stehen muss.
(Funktioniert nur auf der Pages-Website, nicht in der GitHub-Repo-Ansicht –
github.com entfernt JavaScript aus gerendertem Markdown.)
